...
feature_names = df_raw.columns[:-1].tolist() # documentacion
...
with gr.Blocks(title="ML Project App") as app:
    ...
    with gr.Tab("🔢 Modo Manual"):
        ...
        output_m = gr.Text(label="Resultado de la predicción")
    ...
    with gr.Tab("📂 Modo Dataset"):
        ...
        dataset_selector = gr.Dropdown(choices=list_datasets(), label="Selecciona el dataset")
        output_text = gr.Textbox(label="Resumen", interactive=False)
